...
class PersonRegistrationForm(forms.Form):
    ...
    person_second_surname = forms.CharField(label='Segundo apellido', max_length=100, required=False,  validators=[validate_no_numbers])
    person_curp = forms.CharField(label='CURP', max_length=18, required=True, validators=[validate_curp])
    person_city = forms.CharField(label='Ciudad', max_length=100, required=True)
    person_state = forms.CharField(label='Estado', max_length=100, required=True)
    person_email = forms.EmailField(label='Correo electrónico', required=True)
    person_phone = forms.CharField(label='Teléfono', max_length=15, required=True, validators=[validate_phone])
...
class InstitutionRegistrationForm(forms.Form):
    ...
    institution_city = forms.CharField(label='Ciudad', max_length=100, required=True)
    institution_state = forms.CharField(label='Estado', max_length=100, required=True)
    institution_address = forms.CharField(label='Dirección', max_length=255, required=True)
    institution_phone = forms.CharField(label='Teléfono', max_length=15, required=True, validators=[validate_phone])
